...
function( $scope, $rootScope, $http, $timeout, $ionicPopup, $ionicSlideBoxDelegate, $ionicScrollDelegate, config, Api, Nav, leafletData, $state, $stateParams, User, Find, Follow, Marker, $ionicLoading, $ionicModal, localStorageService, $cordovaFileTransfer, $ionicPosition) {
    ...
    $scope.datear.onTagInputBlur = function ($event) {
        $scope.datear.showAutocomplete = false;
    };

    $scope.datear.onTagInputFocus = function ($event) {
        $timeout(function () {
            var pos = $ionicPosition.position(angular.element(document.getElementById('datear-step3')));
            console.log('pos top', pos.top);
            var $scroll = $ionicScrollDelegate.$getByHandle('datear-form-modal');
            $scroll.scrollTo(0, (pos.top + 196), true);
        }, 350);
    };
    ...
    $scope.datear.isValid = function () {
        return !!$scope.datear.selectedTags && $scope.datear.selectedTags.length && !!$scope.datear.object.content;
    };
